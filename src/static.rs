...
use std::{sync::{atomic::{AtomicBool, AtomicU8, AtomicU64, Ordering}, Mutex, Arc}, thread::JoinHandle};
use crate::{message::{MessageToCheck, MessageToPrint, MessageToWrite, MessageToPrintOrigin}, generators::{NumberGenerators, IPGenerator}, display::display};
...
pub static READY___SET_GO_SIGNAL: AtomicBool = AtomicBool::new(false);
...
#[derive(Clone, Debug)]
pub struct ThreadHandler<T: Sized> {
    handles: Arc<Mutex<Vec<JoinHandle<T>>>>
}

impl ThreadHandler<IPGenerator> {
    pub fn new() -> Self { Self { handles: Arc::new(Mutex::new(Vec::new())) } }
    pub fn add(&mut self, item: JoinHandle<IPGenerator>) { self.handles.lock().unwrap().push(item) }
    pub fn unpark(&mut self) {
        if let Ok(unlocked) = self.handles.try_lock() {
            unlocked.iter().for_each(| t: &JoinHandle<IPGenerator> | { t.thread().unpark() })
        }
    }
    pub fn join(self) -> IPGenerator { self.handles.lock().unwrap().pop().unwrap().join().unwrap() }
}

impl ThreadHandler<()> {
    pub fn new() -> Self { Self { handles: Arc::new(Mutex::new(Vec::new())) } }
    pub fn add(&mut self, item: JoinHandle<()>) { self.handles.lock().unwrap().push(item) }
    pub fn unpark(&mut self) {
        if let Ok(unlocked) = self.handles.try_lock() {
            unlocked.iter().for_each(| t: &JoinHandle<()> | { t.thread().unpark() });
        };
    }
    pub fn join_all(self, origin: MessageToPrintOrigin) {
